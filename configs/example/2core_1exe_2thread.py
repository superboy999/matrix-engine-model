'''
Author: superboy
Date: 2024-07-02 17:25:46
LastEditTime: 2024-07-03 03:29:19
LastEditors: superboy
Description: This is for pthread and smt test 
FilePath: /gem5-rvm/configs/example/2core_1exe_2thread.py
...
# GENERAL OPTIONS
ps.add_option('--cmd',              type="string", default="/cluster/home/chenwq/research/rvm/simulator/run/tests/cwq/pthread_test/pthread_test",
                                    help="Command to run on the CPU")
ps.add_option('--numThreads',       type="int", default=2,
                                    help="Number of threads running on the CPU")
ps.add_option('--numCPUs',          type="int", default=2,
                                    help="Number of CPU")
...
ps.add_option('--scale_factor',  type="float", default=1,
                                    help="Data scale factor") #default = 0.0075
...
# MATRIX REGISTER
ps.add_option('--physicReg_num',  type="int", default=16,
...
# MATRIX RENAME
ps.add_option('--numPhysicalRegs',  type="int", default=16,
                                    help="Matrix physical register number")
...
# MATRIX ENGINE TOP
ps.add_option('--lane_num',  type="int", default=8,
...
system = System(
    cpu=[RiscvO3CPU(cpu_id=i) for i in range(options.numCPUs)],
...
Author: superboy
Date: 2024-07-02 17:25:46
LastEditTime: 2024-07-02 21:54:52
LastEditors: superboy
Description: This is for pthread and smt test 
FilePath: /gem5-rvm/configs/example/2core_1exe_2thread.py
...
# system.MatrixCache = [None] * options.numCPUs
# system.memchecker = [None] * options.numCPUs
# system.mem_checker = [None] * options.numCPUs
# setattr(system, "MatrixCache", [None] * options.numCPUs)
# setattr(system, "memchecker", [None] * options.numCPUs)
# setattr(system, "mem_checker", [None] * options.numCPUs)
def createMatrixCache(cpu):
    system.MatrixCache1 = Cache(
        size = options.l1d_size,
        assoc = 4,
        tag_latency = 1,
        data_latency = 1,
...
if( (not connect_to_l1d) and (not connect_to_l2) and (not connect_to_dram)):
    createMatrixCache(system.cpu[i])

# Matrix Extension Configuration
for i in range(options.numCPUs):
    system.cpu[i].matrix_interface = MatrixEngineInterface(
...
def connectMatrixCache(cpu, matrix_engine, l2bus):
    ...
    if(connect_to_l1M):
        matrix_engine.matrix_mmu.matrix_mem_ports = system.MatrixCache[i].cpu_side
        # system.MatrixCache.mem_side = l2bus.cpu_side_ports

for i in range (options.numCPUs):
    connectMatrixCache(system.cpu[i], system.cpu[i].matrix_interface.matrix_engine, system.l2bus, i)
...
for i in range (options.numCPUs):
    system.memchecker[i] = MemChecker()
    system.mem_checker[i] = MemCheckerMonitor(memchecker=memchecker[i])
    system.mem_checker[i].cpu_side_port = system.MatrixCache[i].mem_side
    system.mem_checker[i].mem_side_port = system.l2bus.cpu_side_ports
...
def createMatrixCache(cpu, i):
    system.MatrixCache[i] = Cache(
        size = options.l1d_size,
        assoc = 4,
        tag_latency = 1,
        data_latency = 1,
...
if( (not connect_to_l1d) and (not connect_to_l2) and (not connect_to_dram)):
    for i in range(options.numCPUs):
        createMatrixCache(system.cpu[i], i)
...
def connectMatrixCache(cpu, matrix_engine, l2bus, i):
    cpu.icache_port = cpu.icache.cpu_side
    cpu.dcache_port = cpu.dcache.cpu_side
    cpu.icache.mem_side = l2bus.cpu_side_ports
    cpu.dcache.mem_side = l2bus.cpu_side_ports
...
for i in range (options.numCPUs):
    system.memchecker[i] = MemChecker()
    system.mem_checker[i] = MemCheckerMonitor(memchecker=system.memchecker[i])
    system.mem_checker[i].cpu_side_port = system.MatrixCache[i].mem_side
    system.mem_checker[i].mem_side_port = system.l2bus.cpu_side_ports
